...
def otsu_threshold(image):
    ...
    hist, _ = np.histogram(image, bins=np.arange(0, 256), density=True)
    ...
    num_bins = len(hist)
    class_probabilities = cdf / cdf[-1]
    
    # Calculate the means for each class
    class_means = np.arange(num_bins) * hist
    class_means = class_means.cumsum() / cdf
    
    # Calculate the global mean
    global_mean = class_means[-1]
    
    # Calculate between-class variance
    small_constant = 1e-10  # A small constant to avoid division by zero
    between_class_variance = ((global_mean * class_probabilities - class_means)**2) / (class_probabilities * (1 - class_probabilities) + small_constant)
    ...
    return optimal_threshold, thresholded_image, mask